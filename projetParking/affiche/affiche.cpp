...
void setEclairage(int adress,int valeur){
  ...
  if (valeur==1){
    Wire.write(0xFD);//activation rétro éclairage
  }
  else {
    Wire.write(0xF9);//pas de rétro éclairage
  }
...
void initAfficheur(int adress){
  ...
  envoyerCommande(adress,0xA0); // 1    0    1    0    0    0    0    0 :set Vlcd=>V=0 (set Va) et voltage=32 ; impératif de choisir Va 
  ...
  envoyerCommande(adress,0x02); // 0    0    0    0    0    0    1    0 :return home=> mise à 0 de l'adresse de la DDRAM
...
void effacerAfficheur(int adress){
  ...
  envoyerCommande(adress,0xc0); // 1    1    0   0    0    0    0    0 :set DDRAM=> mise à 0x40 de l'adresse de la DDRAM
  Wire.beginTransmission(adress);
  Wire.write(0x40);
  for (i=0;i<38;i++){
    Wire.write(0xa0);
...
void envoyerMessage(int adress,const char* maChaine, int ligne){
  ...
  if (ligne==LIGNE1){
   ...
   Wire.endTransmission();
    ...
    envoyerCommande(adress,0xC0); // 1    1    0    0    0    0    0    0 :set DDRAM=> va à l'adresse 40 de la DDRAM (2eme ligne)
    Wire.beginTransmission(adress);
    Wire.write(0x40);
    while(*(maChaine+i)!='\0'){
       c=(int)(*(maChaine+i))+0x80;
